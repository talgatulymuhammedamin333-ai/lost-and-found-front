...
function Lost() {
    ...
    const filtered = posts.filter(user => user.title.toLowerCase().includes(search.toLowerCase()))
...
const getPosts = async () => {
    try {
        const result = await axios.get(
            'https://lost-and-found-1fv4.onrender.com/posts/lost'
        );
...
console.log(posts)
  ...
  return (
    <div className='mt-20'>
        <h1 className='text-center text-5xl font-bold'>Жоғалған заттар</h1>
        <div className="flex gap-3 border-b mt-15 border-black pb-8 mb-6">
            <input type="text" onChange={(e) => setSearch(e.target.value)} placeholder="Посттарды іздеу..." className="flex-1 ml-5 p-3 bg-gray-100 border rounded-xl outline-none"/>
        ...
        <div className='grid grid-cols-4 gap-15 mx-10'>
            ...
            <div className="relative">
                <img className="w-full h-52 object-cover" src={`https://lost-and-found-1fv4.onrender.com/${post.image_url}`}  alt=""/>
