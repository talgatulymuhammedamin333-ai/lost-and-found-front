  ...
  function MyProfile() {
    ...
    setPreview(URL.createObjectURL(file));
      ...
      try {
        ...
        const result = await axios.post('http://localhost:3000/profile',{id: 1}, {
          headers: {
    ...
    const updateEdit = async () => {
      ...
      setEdit(false)
      try {
        let result = await axios.put('http://localhost:3000/updateProfile', 
          formData,
          {
            headers: {
              'Content-Type': 'multipart/form-data'
      ...
      <div className=" flex justify-center items-center pb-30">
        <div className="bg-white shadow-xl rounded-3xl w-full max-w-4xl overflow-hidden">
          ...
          <div className="relative flex justify-center">
            ...
            {edit ? <img src={preview || `http://localhost:3000/uploads/${img}`} alt={preview} className="w-32 h-32 rounded-full border-4 border-gray-500 -mt-16 object-cover"/> : <img src={`http://localhost:3000/uploads/${img}`} alt="" className="w-32 h-32 rounded-full border-4 border-gray-500 -mt-16 object-cover"/>}
          </div>
